...
class Solution {
    public int[][] intervalIntersection(int[][] firstList, int[][] secondList) {
        ...
        for(int i=0;i<mergedLen;i++) {
            int right = mergedList[i][1];

            while(i+1 < mergedLen && right >= mergedList[i+1][0]) {            
                result.add(new int[]{mergedList[i+1][0], Math.min(right, mergedList[i+1][1])});
                right = Math.max(right, mergedList[i+1][1]);
                i++;
            }
